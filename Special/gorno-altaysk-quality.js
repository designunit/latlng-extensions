...
command("AddViz", async ctx => {
    return AddFeature(title('Визуально-Эстетический'), SOURCE_ID_VIZ, [
        ...
        ['Количество людей',                 ['number', { label: 'Количество людей', min: 0, max: 100 } ]],
    ])
})
...
command("AddTec", async ctx => {
    return AddFeature(title('Технический анализ'), SOURCE_ID_TEC, [
        ...
        ['Климатические элементы', ['select', { label: 'Состояние элементов городской среды работающей с климатическими особенностями (навесы, защита от ветра, защита от пыли, защита от сели?)' }, rating]],  
    ])
})
